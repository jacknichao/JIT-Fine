...
def mapping_dict_msg(pad_msg, dict_msg):
    ...
    #         print(i)
    new_pad = [
        [dict_msg[w.lower()] if w.lower() in dict_msg.keys() else dict_msg['<NULL>'] for w in line.split(' ')] for line
        in pad_msg
    ]
...
def mapping_dict_code(pad_code, dict_code):
    new_pad = [
        np.array([
            np.array([dict_code[w.lower()] if w.lower() in dict_code else dict_code['<NULL>'] for w in l.split(' ')])
            for l in ml
        ])
        for ml in pad_code]
    return np.array(new_pad)
