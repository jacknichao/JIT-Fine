...
class DeepJITExtended(nn.Module):
    ...
    def forward(self, ftr, msg, code, use_gpu=True):
        if use_gpu:
            x_msg = self.embed_msg(msg.type(torch.LongTensor).cuda())
        else:
            x_msg = self.embed_msg(msg.type(torch.LongTensor))
        x_msg = self.forward_msg(x_msg, self.convs_msg)

        if use_gpu:
            x_code = self.embed_code(code.type(torch.LongTensor).cuda())
        else:
            x_code = self.embed_code(code.type(torch.LongTensor))
