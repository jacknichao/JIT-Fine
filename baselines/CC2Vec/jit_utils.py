...
def mini_batches_DExtended(ids, X_ftr, X_msg, X_code, Y, mini_batch_size=64, seed=0):
    m = X_msg.shape[0]  # number of training examples
    mini_batches = list()
    np.random.seed(seed)

    shuffled_ids, shuffled_X_ftr, shuffled_X_msg, shuffled_X_code, shuffled_Y = ids, X_ftr, X_msg, X_code, Y
    num_complete_minibatches = int(math.floor(m / float(mini_batch_size)))
    ...
    for k in range(0, num_complete_minibatches):
        ...
        mini_batch = (mini_batch_ids, mini_batch_X_ftr, mini_batch_X_msg, mini_batch_X_code, mini_batch_Y)
        mini_batches.append(mini_batch)
